...
class Timer:
    ...
    def run(self):
        print(f"********* Timer TRIGGERED for {self.server_id} of type {self.server_type} for purpose {self.purpose} *********")
        self.callback()
        if self.running:
            self.timer = self.create_timer()
            self.timer.start()
    ...
    def stop(self):
        print(f"********* Timer STOPPED for {self.server_id} of type {self.server_type} for purpose {self.purpose} *********")
        self.running = False
        self.timer.cancel()
